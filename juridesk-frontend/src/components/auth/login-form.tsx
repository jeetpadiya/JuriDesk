...
export function LoginForm() {
  ...
  async function onSubmit(values: LoginValues) {
    ...
    try {
      ...
      if (response.user) {
        useAuthStore.getState().setAuth({ id: response.user.id, email: response.user.email })
      }
      setSubmitSuccess(response.message || "Signed in successfully.")

      const nextPath = new URLSearchParams(window.location.search).get("next") || "/dashboard"
      if (nextPath.startsWith("/")) window.location.assign(nextPath)
    } catch (error) {
  ...
  return (
    <AuthCardWrapper>
        ...
        <form className="space-y-5" onSubmit={form.handleSubmit(onSubmit)} noValidate>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between gap-4">
              ...
              <button type="button" className="text-xs font-medium text-[#946b29] transition hover:text-[#6e4d19]">
                Forgot password?
              </button>
            </div>
            <FormField
      ...
      <p className="mt-8 text-center text-sm text-stone-500">
        New to JuriDesk?{" "}
        <Link href="/signup" className="font-semibold text-[#946b29] transition hover:text-[#6e4d19] hover:underline">
          Create an account
        </Link>
      </p>
    </AuthCardWrapper>
